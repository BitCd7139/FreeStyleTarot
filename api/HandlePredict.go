...
func HandlePredict(c *gin.Context) {
	...
	if _, err := tarot.BuildSpreadMaterials(req.Cards); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	llmResult, err := tarot.RunReadingSync(c.Request.Context(), req)
	if err != nil {
	...
	// 3. 组装最终返回给前端的内容
	finalAnswer := llmResult + "\n\n" + "--- \n" + "### 喜欢的话就来Github点个Star吧！https://github.com/BitCd7139/FreeStyleTarot/tree/main"
