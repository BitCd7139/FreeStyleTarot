...
func scanUser(row interface {
	...
	err := row.Scan(
		...
		&u.Balance, &u.LastPredictAt, &u.ExperienceMode, &u.CreatedAt, &u.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
...
const userColumns = `id, email, nickname, password_hash, tier, vip_expires_at,
	balance, last_predict_at, experience_mode, created_at, updated_at`
...
// UpdateProfile 部分更新用户资料，仅写入非 nil 字段；两者皆 nil 返回错误
func (r *Repository) UpdateProfile(ctx context.Context, id uuid.UUID, nickname *string, experienceMode *string) error {
	if nickname == nil && experienceMode == nil {
	...
	if experienceMode != nil {
		if _, err := db.Pool.ExecContext(ctx,
			`UPDATE users SET experience_mode = $2, updated_at = NOW() WHERE id = $1`,
			id, *experienceMode); err != nil {
			return err
		}
	}
	return nil
