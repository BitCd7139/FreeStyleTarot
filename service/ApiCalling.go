...
func CallDeepSeekStream(ctx context.Context, systemPrompt string, userPrompt string, onChunk func(chunk string) error, maxTokens int, temperature float64) error {
	if globalLlm == nil {
		return fmt.Errorf("LLM client not initialized")
	}
	if maxTokens <= 0 {
		maxTokens = agentPersonaAdvisor.MaxTokens
	}
	if temperature <= 0 {
		temperature = agentPersonaAdvisor.Temperature
	}
