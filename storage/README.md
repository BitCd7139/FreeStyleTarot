...
两次调用共用同一份 **System 提示**（共享规则 + 角色人设），仅在 User 消息开头用 `[ACTIVE AGENT: …]` 切换职责，以便 DeepSeek 等支持**前缀缓存**的接口复用长 System。
...
1. **共享 System**（`tarot_shared_system.md`）  
   韦特/荣格约束、各 Agent 职责与输出结构、角色调度说明。

2. **角色卡**（`prompt_{model}.md`）  
   与前端 `model` 字段对应，如 `default` / `mate` / `neko` / `zako`。拼在共享 System 之后；解读阶段忽略人设，建议阶段才用人设语气。

旧版单 Agent 用的 `global_prompt.md` 已不再参与运行时组装，仅作参考。
...
- **改 Agent 行为**：优先改共享 System 里的职责说明；改语气改对应 `prompt_*.md`。
