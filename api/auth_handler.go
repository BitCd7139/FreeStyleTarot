...
func HandlePatchMe(c *gin.Context) {
	...
	if req.Nickname == nil && req.ExperienceMode == nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "请至少提供一个字段"})
		return
	}

	user, err := authSvc.UpdateProfile(c.Request.Context(), userID, req.Nickname, req.ExperienceMode)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
