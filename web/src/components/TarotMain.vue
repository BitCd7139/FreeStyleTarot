<template>
  ...
  const streamPhaseStreams = ref(false);
  ...
  const onCardPickerConfirm = ({ name, isReversed }) => {
    if (drawnCards.value.length >= 15) return;
  ...
  const submitToBackend = async () => {
    ...
    streamPhaseStreams.value = false;
    ...
    try {
      const stream = predictStream(payload, submitModalRef.value.submitInfo, getToken());
      for await (const event of stream) {
        if (event.type === 'status') {
          streamPhaseLabel.value = event.label;
          streamPhaseStreams.value = !!event.streamPublic;
        } else if (event.type === 'content' && typeof event.content === 'string') {
          if (streamPhaseStreams.value && streamPhaseLabel.value) {
            streamPhaseLabel.value = '';
          }
          backendAnswer.value += event.content;
        }
      }
      await fetchMe();
